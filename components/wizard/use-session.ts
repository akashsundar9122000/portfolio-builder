...
export interface Features { text: boolean; image: boolean }
...
export function useBuilderSession(): { ready: boolean; features: Features } {
  ...
  const [state, setState] = useState<{ ready: boolean; features: Features }>({ ready: false, features: { text: false, image: false } });
  useEffect(() => {
    let live = true;
    void (async () => {
